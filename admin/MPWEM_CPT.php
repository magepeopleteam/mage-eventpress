<?php
	...
	if ( ! class_exists( 'MPWEM_CPT' ) ) {
		class MPWEM_CPT {
			public function __construct() {
				add_action( 'init', [ $this, 'add_cpt' ] );
				add_filter( 'manage_mep_events_posts_columns', [ $this, 'posts_column' ] );
			...
			public function add_cpt() {
				...
				// Register RSVP Responses CPT
				$rsvp_args = array(
					'labels'              => array(
						'name'          => __( 'RSVP Responses', 'mage-eventpress' ),
						'singular_name' => __( 'RSVP Response', 'mage-eventpress' ),
					),
					'public'              => false,
					'has_archive'         => false,
					'exclude_from_search' => true,
			...
			public function posts_column( $columns ) {
				unset( $columns['date'] );
				$columns['mep_status']     = __( 'Elapsed Time', 'mage-eventpress' );
				$columns['mep_event_date'] = __( 'Event Start Date', 'mage-eventpress' );
				$columns['mep_event_seat'] = __( 'Total - Sold - Reserve = Available', 'mage-eventpress' );
				return $columns;
			}
			public function posts_column_data( $column, $post_id ) {
				$post_id = mep_get_default_lang_event_id( $post_id );
                $all_date=MPWEM_Functions::get_dates($post_id);
				$date    = MPWEM_Functions::get_upcoming_date_time( $post_id,$all_date );
				switch ( $column ) {
					case 'mep_status' :
						MPWEM_Layout::elapsed_time_status( $date );
						break;
					case 'mep_event_date' :
						if ( $date ) {
							?>
                            <div class="mpwem_style">
                                <button type="button" class="_button_warning_xxs"><?php echo esc_html( MPWEM_Global_Function::date_format( $date, 'full' ,$post_id) ); ?></button>
                            </div>
							<?php
						}
						break;
					case 'mep_event_seat' :
						MPWEM_Layout::seat_status( $post_id,$date );
						break;
				}
			}
		}
