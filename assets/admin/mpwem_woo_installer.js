...
(function ($) {
	...
	var config    = window.mpwem_woo_installer || {};
	var $overlay  = null;
	var $popup    = null;
	var $btn      = null;
	var $progress = null;
	...
	$(document).ready(function () {
		$overlay  = $('#mpwem-woo-overlay');
		$popup    = $overlay.find('.mpwem-woo-popup');
		$btn      = $('#mpwem-woo-install-btn');
		$progress = $('#mpwem-woo-progress');
		$fill     = $('#mpwem-woo-progress-fill');
		$status   = $('#mpwem-woo-status-text');
		$actions  = $overlay.find('.mpwem-woo-actions');
	...
	function installWooCommerce() {
		$.ajax({
			...
			dataType: 'json',
			data: {
				action: 'mpwem_install_woocommerce',
	...
	function activateWooCommerce() {
		$.ajax({
			...
			dataType: 'json',
			data: {
				action: 'mpwem_activate_woocommerce',
	...
	function setProgress(percent, text) {
		...
		$status.text(text).removeClass('mpwem-success mpwem-error');
	}
	...
	function showSuccess() {
		...
		// Update title
		$popup.find('.mpwem-woo-title').text(config.i18n.success);
		$popup.find('.mpwem-woo-desc').text(config.i18n.redirecting);
