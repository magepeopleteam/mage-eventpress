	...
	function mep_get_all_order_data_and_create_attendee() {
		...
		if ( get_option( 'mep_event_magor_update_3' ) != 'completed' ) {
			...
			foreach ( $orders as $order_id ) {
				...
				$event_info = maybe_unserialize( MPWEM_Global_Function::get_order_item_meta( $item_id, '_event_user_info' ) );
				$event_id = MPWEM_Global_Function::get_order_item_meta( $item_id, 'event_id' ) ? MPWEM_Global_Function::get_order_item_meta( $item_id, 'event_id' ) : 0;
