<?php
	/**
	 * Smart theme speaker list — modern card layout.
	 *
	 * @var int   $event_id
...
?>
<div class="speaker_list mep-speakers">
	<?php foreach ( $speaker_lists as $speaker_id ) :
		...
		?>
		<a class="mep-speaker" href="<?php echo esc_url( $permalink ); ?>">
			<span class="mep-speaker__avatar<?php echo $thumbnail ? ' has-image' : ''; ?>">
				<?php if ( $thumbnail ) : ?>
					<img src="<?php echo esc_url( $thumbnail ); ?>" alt="<?php echo esc_attr( $name ); ?>" loading="lazy" />
				<?php else : ?>
					<span class="mep-speaker__initial"><?php echo esc_html( $initial ? $initial : '?' ); ?></span>
				<?php endif; ?>
			</span>
			<span class="mep-speaker__meta">
				<span class="mep-speaker__name"><?php echo esc_html( $name ); ?></span>
				<?php if ( $role ) : ?>
					<span class="mep-speaker__role"><?php echo esc_html( $role ); ?></span>
				<?php endif; ?>
			</span>
		</a>
	<?php endforeach; ?>
