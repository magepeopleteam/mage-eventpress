	...
	if ( $event_speaker_enabled == 'yes' && is_array( $speaker_lists ) && sizeof( $speaker_lists ) > 0 ) {
        ...
        <div class="speaker_list">
			<?php foreach ( $speaker_lists as $speaker_id ) {
				...
				$permalink = get_the_permalink( $speaker_id );
				if ( ! $permalink ) {
					continue;
				}
				?>
                <a class="speaker_list__item" href="<?php echo esc_url( $permalink ); ?>">
                    <span class="speaker_list__avatar">
						<img src="<?php echo esc_url( $thumbnail ); ?>" alt="<?php echo esc_attr( $title ); ?>" loading="lazy" />
                    </span>
                    <h6 class="speaker_list__name"><?php echo esc_html( $title ); ?></h6>
                </a>
			<?php } ?>
        </div>
		<?php
