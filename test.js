					jQuery(document).ready(function($) {
						if ($('.payment-sub-tabs').length > 0) {
							...
							$('#mep-wc-start-install').click(function() {
								...
								$btn.css('opacity', '0.6');
								$progress.fadeIn(200);
								$status.text( ' . json_encode( __( "Please wait, configuring WooCommerce...", "mage-eventpress" ) ) . ' );
								...
								$.ajax({
									...
									data: {
										...
										nonce: "' . wp_create_nonce("mep_install_wc") . '"
									},
									success: function(response) {
										if (response.success) {
											$status.css('color', '#0f5132');
											$status.text( ' . json_encode( __( "Successfully Activated! Reloading page...", "mage-eventpress" ) ) . ' );
											setTimeout(function() {
												location.reload();
											}, 1500);
										} else {
											$status.css('color', '#dc3545');
											$status.text( ' . json_encode( __( "Error: ", "mage-eventpress" ) ) . ' + (response.data || "Unknown error") );
									...
									error: function() {
										$status.css('color', '#dc3545');
										$status.text( ' . json_encode( __( "A network error occurred. Please try again.", "mage-eventpress" ) ) . ' );
										$btn.prop("disabled", false);
										$btn.css('opacity', '1');
									}
								});
